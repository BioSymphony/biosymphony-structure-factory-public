...
def validate_contract(contract: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    for index, stage in enumerate(stages):
        ...
        output_validation = stage.get("output_validation")
        if not isinstance(output_validation, dict):
            errors.append(f"stage {stage_id or index} output_validation must be an object")
        else:
            if output_validation.get("mode") != "expected_paths_exist":
                errors.append(
                    f"stage {stage_id or index} output_validation.mode must be expected_paths_exist"
                )
            minimum = output_validation.get("minimum_expected_paths")
            if not isinstance(minimum, int) or isinstance(minimum, bool) or minimum <= 0:
                errors.append(
                    f"stage {stage_id or index} output_validation.minimum_expected_paths must be a positive integer"
                )
            elif isinstance(outputs, list) and minimum != len(outputs):
                errors.append(
                    f"stage {stage_id or index} output_validation.minimum_expected_paths "
                    f"must equal expected_outputs count ({len(outputs)})"
                )
...
def validate_progress(contract: dict[str, Any], events: list[dict[str, Any]], require_terminal: bool) -> tuple[list[str], list[str], dict[str, str]]:
    ...
    for event in events:
        ...
        if require_terminal and status == "completed" and isinstance(stage_id, str):
            validation = stages_by_id.get(stage_id, {}).get("output_validation", {})
            minimum = validation.get("minimum_expected_paths")
            observed = event.get("observed_output_count")
            if not isinstance(observed, int) or isinstance(observed, bool):
                errors.append(
                    f"progress line {event.get('_line')} completed {stage_id} without integer observed_output_count"
                )
            elif isinstance(minimum, int) and observed < minimum:
                errors.append(
                    f"progress line {event.get('_line')} completed {stage_id} with "
                    f"observed_output_count={observed}, expected at least {minimum}"
                )
...
def emit_event(
    ...
    status: str,
    message: str = "",
    observed_output_count: int | None = None,
) -> dict[str, Any]:
    event = {
        "schema_version": 1,
    ...
    if observed_output_count is not None:
        event["observed_output_count"] = observed_output_count
    return event
